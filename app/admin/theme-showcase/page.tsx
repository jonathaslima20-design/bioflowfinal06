...
export default function AdminThemeShowcasePage() {
  ...
  async function moveLanding(themeKey: string, dir: -1 | 1) {
    ...
    const reordered = ordered.map(p => ({ ...p }));
    reordered[idx] = { ...a, landing_order: b.landing_order };
    reordered[swap] = { ...b, landing_order: a.landing_order };
    const newMap = { ...presets };
    for (const p of reordered) newMap[p.theme_key] = p;
    setPresets(newMap);
    await Promise.all([
      upsertShowcasePreset(newMap[a.theme_key]),
      upsertShowcasePreset(newMap[b.theme_key]),
    ]);
  }
